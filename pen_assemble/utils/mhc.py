...
CALLING CONVENTION (Item 4 - must be consistent with PEN-SCORE S_Immuno pipeline):
=================================================================================
PEN-SCORE (pen-score v0.1.0) computed S_Immuno via DIRECT subprocess calls to the
...
PEN-ASSEMBLE MUST use the same calling convention and output parser to ensure epitope
counts are comparable across papers (the IS621 S_Immuno=0.7594 baseline was computed
with PEN-SCORE's direct-subprocess approach).
...
same netMHCpan binary, BUT mhctools uses a different calling convention:
  - Binary name: 'NetMHCpan' (uppercase N) vs PEN-SCORE's 'netMHCpan-4.1'
  - Output format: mhctools returns BindingPrediction objects; PEN-SCORE parsed
    raw TSV with '%Rank_EL' column and IC50_nM threshold of 500 nM
  - IC50 thresholding: mhctools defaults differ from PEN-SCORE's 500 nM strong-binder cutoff

DECISION: PEN-ASSEMBLE uses the same direct subprocess approach as PEN-SCORE (see
...
mhctools and breaking comparability with PEN-SCORE scores.

See also: PEN-SCORE execution summary Deviation D4 (MHCflurry 2.0 rejected as
...
# Allele sets used in PEN-SCORE S_Immuno (locked - must match for comparability).
...
# %Rank_EL thresholds - PEN-SCORE calls (NOT IC50 nM).
RANK_EL_THRESHOLD_I = 0.5  # Class I: %Rank_EL < 0.5 -> strong binder
RANK_EL_THRESHOLD_II = 10.0  # Class II: %Rank_EL <= 10.0

# PEN-SCORE S_Immuno normalization constant (95th-pct across 28 editors, locked).
...
def find_mhc_i_binders(
    ...
    _alleles = alleles or MHC_I_ALLELES

    # Resolve binary: explicit path > PATH lookup > PEN-SCORE VM default
    home = Path.home()
...
def find_mhc_ii_binders(
    ...
    _alleles = alleles or MHC_II_ALLELES
...
def compute_weighted_epitope_load(
    ...
    Matches PEN-SCORE Strategy C optimization target (Script 14 formula).
...
def compute_s_immuno(
    n_i: int,
    n_ii: int,
    max_total: float = S_IMMUNO_MAX_TOTAL,
) -> float:
    """Compute S_Immuno using PEN-SCORE absolute normalization formula.
    ...
    max_total : float
        Normalization constant = 253.5 (95th-pct across 28 editors, locked PEN-SCORE).
