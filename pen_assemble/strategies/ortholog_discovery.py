...
Mines MECH-CLASS's 31,871-protein IS110 triage catalog for novel programmable
...
# (gate 6). Sourced from PEN-SCORE editor_universe.yaml v1.0.4 + ISfinder IS110 list.
KNOWN_IS110_ACCESSIONS: list[str] = [
    "A0A2X3M8B0",  # IS621
    "A0A2X3M8B1",  # IS621_2 (inactive but still a known member)
    # Additional known IS110 members will be loaded from GENOME-ATLAS ATLAS at runtime.
...
def apply_gate_2(df: pd.DataFrame) -> pd.DataFrame:
    ...
    NOTE: In the MECH-CLASS IS110 triage catalog (MECH-CLASS v0.5.1), tier_a is
    ...
    result = df[mask].copy()
    # NOTE: tier_a_confidence is NOT used here because in the MECH-CLASS IS110 triage catalog
...
def apply_gate_6(
    ...
    atlas_embeddings_path: path to GENOME-ATLAS ATLAS DuckDB or embeddings parquet
    ...
    if embeddings is None:
        print(
            ...
            "    Provide --atlas_embeddings path to GENOME-ATLAS ATLAS DuckDB to enable.\n"
...
def apply_gate_7(
    ...
    ** IS110 triage catalog fast-path **
    The MECH-CLASS IS110 triage catalog (is110_triage.parquet) was built by MECH-CLASS
    ...
    already passed an equivalent of Gate 7 via MECH-CLASS's analysis.  When this column
...
def _normalize_catalog_columns(df: pd.DataFrame, source_label: str) -> pd.DataFrame:
    """Rename MECH-CLASS catalog columns to the internal schema expected by gate functions.

    MECH-CLASS parquet files use:
...
def run_strategy_b(
    ...
    if fanzor_catalog_path and fanzor_catalog_path.exists():
        ...
        fanzor["source"] = "fanzor"
        is110["source"] = "is110_triage"
