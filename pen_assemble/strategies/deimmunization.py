...
MHC calling convention:
  ...
  Matches Paper 3 calling convention for comparability.
...
def _check_mhciipan_available() -> bool:
    """Return True if the Paper 3 VM NetMHCIIpan-4.0 Perl script is accessible."""
...
def _run_netmhciipan(
    sequence: str,
    alleles: list[str],
    rank_threshold: float = 10.0,  # Paper 3: %Rank_EL <= 10.0 for Class II
) -> tuple[int, list[int]]:
...
def compute_epitope_profile(
    ...
    Uses Paper 3 calling convention when use_netmhcpan=True:
...
def calibrate_s_immuno(
    ...
    """Map weighted epitope load to S_Immuno using Paper 3 absolute normalization.

    Paper 3 formula (Script 14):
        combined    = n_I + 0.5 * n_II   (= variant_weighted_load)
        S_Immuno    = 1 - combined / max_total
        max_total   = 253.5  (95th-pct across 28 editors, locked Paper 3)
    ...
    wt_weighted_load : float
        Kept for API compatibility; NOT used in the Paper 3 formula.
    wt_s_immuno : float
        Kept for API compatibility; NOT used in the Paper 3 formula.
    max_total : float
        Paper 3 normalization constant (253.5, do not change).
    """
...
def run_mc_trajectories(
    ...
    Hybrid evaluation strategy:
      - MC inner loop uses the Parker immunogenicity HEURISTIC for scoring (fast).
        This avoids 2000 × n_trajectories netMHCpan calls (would take >24 h).
      - WT baseline and FINAL variant sequences are evaluated with real
        netMHCpan-4.1 / NetMHCIIpan-4.0 (Paper 3 calling convention).
      - Total real MHC calls: 1 (WT) + n_trajectories (final eval) ≈ 31 calls
        for 10 variants × 3x over-sampling → ~2-5 minutes on the VM.

    This ensures:
      - MC guidance is quick enough to be practical
      - All reported S_Immuno values are real netMHCpan outputs (not heuristic)
      - Comparability with Paper 3 IS621 WT baseline (n_I=15, n_II=92) is maintained
    """
