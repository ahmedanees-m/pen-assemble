...
Reuses all 7 PEN-SCORE axis pipelines.

IS621 lockpoint: PenScore = 0.929.
...
S_Mature inheritance rules (pre-registered before any design generation):
  ...
  Strategy D ProteinMPNN (IS621 backbone)   : S_Mature = IS621_S_Mature x 0.5 = 0.396
...
The 7 axes and their computation sources:
  ...
  S_Cargo   : enzyme mechanism gate (transposase/recombinase -> 1.0 per PEN-SCORE rules)
  S_Deliv   : protein size scoring (target: <=1200 aa; AAV capacity)
  S_Immuno  : netMHCpan epitope load (calibrated to PEN-SCORE scale)
  S_Prog    : programmability model (RNA-guide inference from sequence)
  S_Mature  : citations x expertise inheritance (pre-registered rules above)
"""
...
IS621_PENSCORE = 0.929  # P1 lockpoint
IS621_S_MATURE = 0.792  # PEN-SCORE v0.1.0 public_scorecard.parquet
IS621_S_IMMUNO = 0.7594  # PEN-SCORE v0.1.0 (P3 deimm baseline)
IS621_S_DELIV = 0.949  # estimated from PEN-SCORE (~260 aa, AAV-compact)
...
# Axis weights (from PEN-SCORE pen-score v0.1.0; must be identical for comparability)
