...
**Pre-registered predictions verified**: 5/5 PASS — *"PUBLISH with strong claim"*
...
| B | IS110-family ortholog survey (Paper 2 catalog) | 992 | 992 |
...
| `ddg_kcal_mol` | Rosetta ΔΔG value (see Deviation 5 — non-functional gate) |
...
1. **Rosetta ΔΔG gate non-functional** (Deviation 5): ALL 47 computed ΔΔG values
   ...
   Gate auto-passed; structural quality proxied by ESMFold pLDDT.
...
3. **P3 reference corrected**: Execution plan placeholder IS621 S_Immuno = 0.250
...
5. **Gate 8 (ATLAS novelty) not evaluated** for Strategy B: Paper 1 ATLAS DuckDB
...
```
/home/anees_22phd0670/esm_tier1_output/pdbs/<design_id>.pdb
```

The `final_pdb` column in `pen_assemble_catalog.csv` stores the full VM path.  
Transfer via SFTP (paramiko; chunked 32 KB reads; `window_size = 2**27`) when preparing
the Zenodo deposit. See `STRUCTURES_NOTE.txt` for details.
...
> *[journal pending]*. Pipeline v0.5.0. DOI: [pending Zenodo deposit].
