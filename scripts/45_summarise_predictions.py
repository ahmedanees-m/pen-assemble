...
This script is Step 22.5 of PEN-ASSEMBLE Part E. It loads the five
individual JSON result files produced by scripts 40-44 and applies
the publication policy from the execution plan (§0.4).
...
DEVIATIONS = {
    ...
    "p3_wt_reference_corrected": (
        "Execution plan placeholder WT_IS621_S_IMMUNO=0.250 corrected to "
        "Paper 3 published value 0.7594."
    ),
...
def publication_policy(n_pass: int, n_fail: int, n_ne: int) -> str:
    n_eval = n_pass + n_fail
    if n_pass == 5:
        return "PUBLISH with strong claim -- 5/5 PASS"
    elif n_pass == 4 and n_eval == 4 and n_ne == 1:
        return "PUBLISH with honest report -- 4/4 evaluated PASS, 1 NOT_EVALUABLE"
    elif n_pass == 4 and n_fail == 1:
        return "PUBLISH with honest report -- 4/5 PASS, 1 FAIL"
    elif n_pass == 3:
        return "PUBLISH with reframed contribution -- 3/5 PASS"
    else:
...
def main() -> None:
    ...
    policy = publication_policy(n_pass, n_fail, n_ne)

    # ── Print summary ─────────────────────────────────────────────────────────
    print("\n" + "="*65)
    print("PRE-REGISTERED PREDICTION SUMMARY (PEN-ASSEMBLE Part E)")
    print("="*65)
    labels = {
    ...
    for key, msg in DEVIATIONS.items():
        print(f"    [{key}]: {msg[:95]}...")
