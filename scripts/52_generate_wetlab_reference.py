...
def make_design_md(row: pd.Series, top5_ids: set[str]) -> str:
    ...
    ddg_display = (f"{ddg:.1f} kcal/mol *(cross-protein absolute energy; not true ΔΔG — see Deviation 5)*"
                   if ddg is not None and not (isinstance(ddg, float) and ddg != ddg)
                   else "not computed (Rosetta gate non-functional — see Deviation 5)")
    ...
    lines.append(f"| PDB path (VM)      | `/home/anees_22phd0670/esm_tier1_output/pdbs/{did}.pdb` |")
    lines.append("")
