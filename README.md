...
> **Part of PEN-STACK** - a computational biology pipeline for programmable genome editor discovery.
> [genome-atlas](https://github.com/ahmedanees-m/genome-atlas) → [mech-class](https://github.com/ahmedanees-m/mech-class) → [pen-score](https://github.com/ahmedanees-m/pen-score) → **pen-assemble** → PEN-COMPARE
...
**16 designs beat IS621** (the current gold-standard bridge recombinase) on the pre-registered PenScore threshold of 0.929. The top candidate - a Monte Carlo deimmunized IS621 variant - scores **0.9673**, combining a +11.8% improvement in immunogenicity over IS621 while preserving full mechanistic activity. All five pre-registered predictions **PASS**.
...
| **Reproducibility researchers** | Audit the complete pre-registration, evaluation, and re-scoring chain. Every decision is documented. |
...
```
  ...
  │                 │   use-case profile  │  S_DSB    x 0.24  (DSB avoidance)    │
  │  Writer scoring │                     │  S_Spec   x 0.14  (targeting prec.)  │
  │  framework      │                     │  S_Cargo  x 0.19  (payload capacity) │
  │  IS621 = 0.957  │                     │  S_Deliv  x 0.19  (AAV compat.)      │
  │  SpCas9 = 0.402 │                     │  S_Immuno x 0.09  (immunogenicity)   │
  └─────────────────┘                     │  S_Prog   x 0.05  (programmability)  │
                                          │  S_Mature x 0.05  (tech. maturity)   │
                                          │  S_Energy x 0.05  (ATP independence) │
                                          └─────────────────┬─────────────────────┘
                                                            │
                                                            ▼
                                          ┌───────────────────────────────────────┐
...
PEN-ASSEMBLE is the last stage of the PEN-STACK pipeline. Each package provides critical inputs to the next:

| Package | What it does | Key result | Version |
|---------|-------------|------------|---------|
...
```
PenScore = S_DSB x 0.24  +  S_Spec x 0.14  +  S_Cargo x 0.19
         + S_Deliv x 0.19 +  S_Immuno x 0.09 +  S_Prog x 0.05
         + S_Mature x 0.05 +  S_Energy x 0.05
```
...
├── DESIGN_PROVENANCE.md             # Methodology and limitations notes
...
Full methodology in [`DESIGN_PROVENANCE.md`](DESIGN_PROVENANCE.md).

| # | Item | Effect |
|---|------|--------|
| 1 | The Rosetta stability gate returns absolute energies rather than comparable delta-delta-G values, so it cannot threshold across sequences. | ESMFold pLDDT (>= 90 global, >= 95 active-site) is used as the structural-quality proxy instead. |
| 2 | The IS621 S_Immuno baseline is the accurate computed value (0.7594, MHCflurry). | P3 is evaluated from the accurate baseline. |
| 3 | The top-5 is diversity-constrained: A_007 (0.9209) takes the rank-5 slot over D023 (0.9319) to satisfy the >= 3-strategy criterion. | P5 holds; the rank-5 substitution is noted. |
| 4 | MHCflurry 2.2.1 recalibration shifts IS621 S_Immuno to 0.7243 (vs 0.7594 baseline). | Calibrated threshold = 0.9255; a secondary analysis finds 32 designs above it. |
| 5 | Domain-based classifiers can mis-call IS110 proteins as DSB_NUCLEASE. | Resolved by the PF01548 + PF02371 biochemical gate in mech-class. |
...
```bash
# Full catalog assembly (produces catalog/release_v0.5.0/)
...
**PEN-STACK** (*Programmable Enzyme Networks - Systematic Tool for Atlas and Knowledge*) is a computational biology pipeline for programmable genome editor discovery and benchmarking.

| Package | Role in the pipeline | Version |
|---------|---------------------|---------|
| [**genome-atlas**](https://github.com/ahmedanees-m/genome-atlas) | Foundational knowledge graph of 28 enzyme systems (AUROC 0.9714) | v0.7.2 |
| [**mech-class**](https://github.com/ahmedanees-m/mech-class) | Biochemical mechanism classifier - IS110 Tier-A gate (F1 = 0.986) | v0.5.4 |
| [**pen-score**](https://github.com/ahmedanees-m/pen-score) | 8-axis writer scoring framework - IS621 = 0.957, SpCas9 = 0.402 | v0.1.3 |
| **pen-assemble** *(this repo)* | IS110-family design catalog - 1,029 designs, 5/5 predictions PASS | **v0.5.2** |
| **PEN-COMPARE** | Cross-system benchmarking - TrueWriterScore 6-gate classifier | - |
...
> Bridge-recombinase Library Engineering. v0.5.2. https://github.com/ahmedanees-m/pen-assemble
